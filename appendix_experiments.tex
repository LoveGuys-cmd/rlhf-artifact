...
The aggregate tie-adjusted preference scores against Mean-PPO are 0.494
([0.478, 0.510]) for Qwen and 0.502 ([0.458, 0.544]) for ArmoRM, where the
intervals are three-seed hierarchical-bootstrap 95\% intervals.  The frozen
gate requires a mean at least 0.50 and a lower endpoint at least 0.45; thus
ArmoRM passes and Qwen fails by the predeclared mean threshold.  The combined
model-based anti-exploitation battery is consequently false.  A target-RM
increase accompanied by proxy degradation is reported as evidence consistent
with reward-model exploitation, while a passing proxy can only support
``no evidence under the measured evaluators,'' not its absence.
