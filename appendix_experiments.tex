...
The frozen ordinal calibration report uses the simultaneous one-sided level
\(\alpha_{\rm cal}=0.05\) and records
\(\varepsilon_{\rm cal}=0.0468083\).  Its prompt-cluster count and hash are
preserved in the machine-readable calibration report consumed by the training
and verification scripts; the value is fixed before any policy run.
...
and GRPO in every seed, while Scalar max-PPO is lower in two of the three
seeds.  Its robust maximum remains competitive across the same paired prompts.
...
\((3.467,0.048)\), while removing calibration gives
...
sensitivity comparisons rather than isolated causal effects.  Calibration
removal changes the evaluator-defined target; floor removal changes the
accepted-update trajectory.  The records therefore support a controlled
frontier interpretation, not a claim that any safeguard is a universally
...
Both scores are near parity and both intervals contain 0.5, so neither
evaluator shows a clear quality-degradation signal relative to Mean-PPO.  A
