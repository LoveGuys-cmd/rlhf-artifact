...
while standard RLHF optimizes the expected reward of a single response.
Recent finite-budget objectives partly close this gap, but typically
reduce each response to a scalar reward and discard the per-rating-level
probabilities that determine the maximum. This matters because two responses with the same predicted mean
can have different probabilities of receiving a high rating, and thus
different expected latent rating maxima within a candidate group. We introduce \textbf{\method{}}, which directly optimizes the finite-budget
expected rating maximum using the predictive distribution of a frozen ordinal reward model
trained from human ratings. Its variance is retained as a model-based diagnostic of annotator disagreement,
not an optimization bonus. A one-sided CDF ambiguity set with a fixed
radius yields the exact worst-case expected latent rating maximum and an exact
response-level policy credit for PPO. To limit degradation in typical responses and policy drift, we impose lower
bounds on the predicted mean rating and on an independently trained quality
score, and regulate KL divergence from the reference policy with rollback. Across three seeds at
\(N=32\), \textbf{\method{}} achieves competitive robust maximum
performance with lower held-out KL than Mean-PPO, GRPO, and scalar
max-PPO baselines. The max-oriented baselines achieve higher robust maxima;
our results establish a lower-drift operating point under the frozen evaluators,
not uniform improvement across endpoints.
\end{abstract}
\section{Introduction}
\label{sec:introduction}

Many language-model systems generate multiple responses and then return, rerank,
or inspect the best candidate \citep{nakano2021webgpt,cobbe2021training},
whereas standard RLHF objectives optimize the expected score of a single
response \citep{christiano2017deep,stiennon2020learning,ouyang2022training}. This mismatch is consequential: a higher
mean score need not increase the probability that a fixed generation budget
yields a highly rated response. Mean-PPO and GRPO remain effective
general-purpose post-training methods
\citep{shao2024deepseekmath,guo2025deepseekr1}, and recent
$\mathrm{max}@k$, Best-of-\(N\), and inference-aware policy-gradient methods
better align training with finite-budget deployment
\citep{chow2025inference,tang2025inference,walder2025pkpo,
bagirov2025best,li2026posttraining,poiani2026maxk}. In the scalar-reward formulations of these objectives, each response is
represented by a single score,
discarding the per-rating-level probabilities that determine the maximum. This
loss is not innocuous: two responses with the same predicted mean can have
different probabilities of receiving a high rating, and thus different
expected latent rating maxima. Moreover, optimizing a nominal upper tail can amplify
calibration error, degrade typical responses, or drive the policy far from its
reference \citep{gao2023scaling,kirk2024understanding}. We therefore study a
finite-budget expected rating maximum that retains the evaluator's ordinal
information, with quality and policy drift reported separately.
...
fixed envelope on the CDFs in the direction adverse to upper-tail
...
This shared construction is the main technical distinction: CDF
robustification changes both the deployment-level group objective and the credit that
...
frozen Quality RM from preference pairs constrain typical model-scored
quality beyond the target evaluator; reference-policy KL
...
consequently remain separate quantities, making the max--stability
operating point observable rather than reporting a single composite score.

Our contributions connect the distributional information lost by scalar scoring
to an exact objective, an implementable credit, and a controlled empirical test:
\begin{itemize}
\item \textbf{An ordinal interface for finite-budget alignment.}
We retain all five rating probabilities of a frozen reward model trained on
repeated human annotations. This exposes the threshold events needed by the
expected maximum; the mean measures typical model-scored quality and the
variance remains a disagreement diagnostic. The contribution is the use of
this interface for policy optimization, not a new ordinal regression model.
...
ambiguity set and its closed-form leave-one-out credit
(\Cref{thm:main-max,thm:main-gradient}). The score-function estimator is
unbiased before PPO clipping, and a lower bound on the true conditional maximum
holds when the envelope covers the conditional rating distributions
(\Cref{thm:coverage-summary}). Both value and credit are computed in
\(O(N|\mathcal R|)\) time without sampling latent ratings.

\item \textbf{A constrained PPO realization with explicit safeguards.}
We combine this credit with nominal-mean and independently trained quality
floors, reference-policy KL control, and state-complete rollback. The floors
enter through adaptive dual weights rather than fixed reward bonuses;
rollback restores rejected policy and optimizer updates and leaves controller
state unchanged. These mechanisms make the target and its safeguards
separately inspectable without asserting a population KL guarantee.

\item \textbf{A three-seed characterization at a fixed response budget.}
At \(N=32\), \method{} attains a robust maximum of 3.458 with held-out KL
of 0.034, compared with 3.424 and 0.070 for Mean-PPO. GRPO and scalar
max-PPO attain higher maxima at higher drift. Matched comparators, component
ablations, and independent model-based diagnostics characterize this operating
point; they do not establish a complete Pareto frontier or superiority on
...
whereas the models, CDF radius, floors, soft target \(\delta\), and hard
...
produces a reproducible operating point that is both robust and stable.
...
into one weighted score would conceal the robust--stable operating point the
...
effects.  In particular, removing CDF robustification changes the evaluator-defined
