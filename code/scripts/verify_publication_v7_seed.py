#!/usr/bin/env python3
"""Fail closed unless one publication-v7 seed is technically complete."""
...
def verify_training(root: Path, seed: int, steps: int, hard_kl: float) -> dict[str, Any]:
    ...
    for method in TRAIN_METHODS:
        ...
        for row in rows:
            ...
            restored = finite(row, "reference_kl")
            ...
            if was_rejected:
                if attempted <= hard_kl or restored > hard_kl + 1e-6:
...
def main() -> None:
    ...
    protocol = read_json(args.protocol_dir / "PUBLICATION_PROTOCOL_V7.json")
    if protocol.get("protocol") != "stablemax-ppo-publication-v7-frozen-before-training":
    ...
    frozen_hashes = verify_frozen_inputs(
        args.repo, args.protocol_dir / "FROZEN_INPUTS_V7.sha256"
    )
    ...
    report = {
        "protocol": "stablemax-ppo-publication-v7-seed-terminal",
        "passed": True,
        "seed": args.seed,
        "steps": args.steps,
